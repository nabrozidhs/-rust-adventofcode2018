...
fn day21(program: &Vec<(String, Vec<i64>)>, pointer: usize, registers: &mut Vec<i64>) -> i64 {
    ...
    while registers[pointer] >= 0 && (registers[pointer] as usize) < program.len() {
        if registers[pointer] == 28 && !seen.insert(registers[5]) {
            break;
        }
...
fn main() {
    ...
    println!("part1 {:?}", day21(&program, pointer, &mut vec![6778585, 0, 0, 0, 0, 0]));
    println!("part2 {:?}", day21(&program, pointer, &mut vec![6534225, 0, 0, 0, 0, 0]));
}
